...
module.exports = {
  ...
  // Database
  database: {
    uri: process.env.MONGODB_URI || 'mongodb://localhost:27017/dreamhouse',
    options: {
      // Mongoose 6+ enables the new URL parser and unified topology by default.
      // Do not pass deprecated connection options.
      maxPoolSize: 10,
  ...
  // JWT
  // Note: test suite sets JWT_SECRET and JWT_REFRESH_SECRET.
  // Some parts of the codebase generate/verify refresh tokens directly using
  // config.jwt.refreshSecret, so ensure it is present and loaded.
  jwt: {
  ...
  // Payments (optional; keep safe defaults so tests/dev don't crash)
  razorpay: {
