...
/**
   * Get booking by ID
   * Authorization: only the owning user, the assigned engineer, or an admin
   * may view the booking. Prevents IDOR (cross-user/cross-engineer access).
   */
  async getBookingById(bookingId, requesterId = null, requesterRole = null) {
    ...
    // Require an authenticated requester. Admins may view any booking.
    if (requesterRole !== 'admin') {
      const isOwner =
        booking.userId &&
        booking.userId._id &&
        booking.userId._id.toString() === requesterId?.toString();
      const isEngineer =
        booking.engineerId &&
        booking.engineerId._id &&
        booking.engineerId._id.toString() === requesterId?.toString();
...
/**
   ...
   * Only the engineer assigned to the booking, or an admin, may update status.
   */
  async updateStatus(bookingId, status, additionalData = {}, userId = null, userRole = null) {
    if (!mongoose.Types.ObjectId.isValid(bookingId)) {
    ...
    // Authorization: only the assigned engineer or an admin may update status.
    if (userId && userRole !== 'admin') {
      const isEngineer = booking.engineerId && booking.engineerId.toString() === userId.toString();
      if (!isEngineer) {
