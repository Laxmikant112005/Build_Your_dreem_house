 ...
 * may view a booking. Prevents IDOR (cross-user/cross-engineer access).
 */
const getBookingById = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const booking = await bookingService.getBookingById(id, req.userId, req.user.role);
...
const updateBookingStatus = asyncHandler(async (req, res) => {
  ...
  const booking = await bookingService.updateStatus(id, status, additionalData, req.userId, req.user.role);
