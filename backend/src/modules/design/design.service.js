...
class DesignService {
  ...
  async getDesignById(designId, incrementView = false) {
    if (!mongoose.Types.ObjectId.isValid(designId)) {
      console.error("DESIGN SERVICE ERROR: Invalid designId format", designId);
    ...
    if (!design) {
      console.error("DESIGN SERVICE ERROR: Design not found", designId);
  ...
  async updateDesign(designId, engineerId, updateData) {
    if (!mongoose.Types.ObjectId.isValid(designId) || !mongoose.Types.ObjectId.isValid(engineerId)) {
      console.error("DESIGN SERVICE ERROR: Invalid ID in updateDesign", {designId, engineerId});
    ...
    if (!design) {
      console.error("DESIGN SERVICE ERROR: Design not found/unauthorized", designId, engineerId);
  ...
  async deleteDesign(designId, engineerId) {
    if (!mongoose.Types.ObjectId.isValid(designId) || !mongoose.Types.ObjectId.isValid(engineerId)) {
      console.error("DESIGN SERVICE ERROR: Invalid ID in deleteDesign", {designId, engineerId});
      throw new ApiError(400, 'Invalid ID format');
    }
    ...
    if (!design) {
      console.error("DESIGN SERVICE ERROR: Design not found/unauthorized for delete", designId);
      throw new ApiError(404, 'Design not found or unauthorized');
    }
