...
const createBooking = [
  ...
  body('startAt')
    ...
    .isISO8601()
    .withMessage('Please provide a valid ISO8601 start datetime'),

  body('endAt')
    .optional()
    .isISO8601()
    .withMessage('Please provide a valid ISO8601 end datetime'),
  ...
  // Either endAt or duration must be provided
  body()
    .custom((value, { req }) => {
      if (!req.body.endAt && !req.body.duration) {
        throw new Error('Either endAt or duration is required');
      }
      return true;
    }),
...
const updateBooking = [
  ...
  // If startAt provided, require either endAt or duration
  body()
    .custom((value, { req }) => {
      if (req.body.startAt && !req.body.endAt && !req.body.duration) {
        throw new Error('When updating startAt, provide endAt or duration');
      }
      return true;
    }),
