...
describe('AUTHENTICATION API TESTS', () => {
  ...
  describe(`POST ${BASE_URL}/auth/register`, () => {
    ...
    it('should fail with duplicate email (409 with field info)', async () => {
      const response = await request(app)
        .post(`${BASE_URL}/auth/register`)
        .send(testUser)
        .expect('Content-Type', /json/);
      ...
      // Verify structured error response with field-specific detail
...
describe('DESIGN API TESTS', () => {
  // The engineer token must be generated AFTER `testEngineerId` is set
  // (in the ENGINEER API TESTS beforeAll). Generating it at module scope
  // would sign an empty `id` and produce a token that fails auth.
  let engineerToken = '';

  beforeAll(() => {
    engineerToken = generateTestToken(testEngineerId, 'engineer');
  });
...
it('should fail with invalid ID', async () => {
      ...
      // The design route uses express-validator `validate`, which returns
      // 422 for malformed ObjectId params.
      expect(response.status).toBe(422);
    });
  });
});
...
describe('BOOKING API TESTS', () => {
  ...
  describe(`POST ${BASE_URL}/bookings`, () => {
    ...
    it('should fail without required fields', async () => {
      ...
      // The booking validator (Joi) enforces required fields and returns
