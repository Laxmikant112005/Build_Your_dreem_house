...
const DesignDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // designService.getById returns the axios body wrapper
        // { success, message, data: design }.
        const res = await designService.getById(id);
        const designData = res?.data?.data ?? res?.data ?? res;
        if (designData) {
          setDesign(designData);
          // engineerService.getAllEngineers returns the axios body wrapper
          // { success, message, data: { engineers, pagination } }.
          const engRes = await engineerService.getAllEngineers();
          const engineersData = engRes?.data?.data?.engineers ?? engRes?.data?.engineers ?? engRes?.engineers ?? [];
          ...
          {selectedEngineer && (
            <div className="bg-white p-8 rounded-4xl border border-slate-200 shadow-sm flex flex-col items-center text-center">
              ...
              <div className="w-24 h-24 rounded-full border-4 border-gold/20 p-1 mb-4 shadow-lg">
                <img src={selectedEngineer?.avatar} alt={selectedEngineer?.name} className="w-full h-full rounded-full object-cover" />
              </div>
              <h3 className="text-xl font-extrabold text-navy mb-1">{selectedEngineer?.name || "Professional Engineer"}</h3>
              <p className="text-slate-400 font-medium text-sm mb-6 uppercase tracking-tight">{selectedEngineer?.specialization || "Structural Engineer"}</p>
