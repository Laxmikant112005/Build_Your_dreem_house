...
import { Save, Edit3, Trash2, MapPin } from 'lucide-react';
...
useEffect(() => {
    fieldService.getUserFields()
      .then((res) => {
        // fieldService.getUserFields returns the axios body wrapper
        // { success, message, data: fields[] }.
        const fields = res?.data?.data ?? res?.data ?? res ?? [];
        if (Array.isArray(fields) && fields.length > 0) {
          const field = fields.find(f => f.isPrimary) || fields[0];
          setFieldData(field);
          const coords = field?.geoJSON?.coordinates?.[0] || [];
          if (Array.isArray(coords) && coords.length > 0) {
            setFieldPoints(coords.map(c => [c[1], c[0]]));
          }
          const loc = field?.address?.location?.coordinates;
          if (Array.isArray(loc) && loc.length >= 2) {
            setPosition([loc[1], loc[0]]);
          }
        }
      })
      .catch(() => {})
