...
const Bookings = () => {
  ...
  const refetch = async () => {
    ...
    setLoading(true);
    try {
      const res = await bookingService.getByUser(user.id);
      // bookingService.getByUser returns the axios body wrapper
      // { success, message, data: bookings[] } (paginated under res.data.bookings).
      const unwrapped = res?.data?.bookings ?? res?.data ?? res ?? [];
      setBookings(Array.isArray(unwrapped) ? unwrapped : []);
